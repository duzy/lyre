...
//#include "lyre/lex/CodeCompletionHandler.h"
//#include "lyre/lex/LexDiagnostic.h"
//#include "lyre/lex/LiteralSupport.h"
//#include "lyre/lex/Preprocessor.h"
...
Lexer::Lexer(LangOptions &Opts, SourceManager &SM, 
  FileID FID, const llvm::MemoryBuffer *InputFile /*, Preprocessor &PP*/)
  : FileLoc(SM.getLocForStartOfFile(FID)), LangOpts(Opts) {
...
///
Lexer *Lexer::Create_PragmaLexer(LangOptions &Opts, SourceManager &SM,
  ...
  Lexer *L = new Lexer(Opts, SM, SpellingFID, InputFile);
...
#else
  assert(false && "This doesn't work on raw lexers");
  return SourceLocation();
#endif
}
...
DiagnosticBuilder Lexer::Diag(const char *Loc, unsigned DiagID) const {
  assert(Diags && "Invalid diagnostic object!");
  return Diags->Report(getSourceLocation(Loc), DiagID); //PP->Diag(getSourceLocation(Loc), DiagID);
}
...
static char DecodeTrigraphChar(const char *CP, Lexer *L) {
  ...
  if (!L->getLangOpts().Trigraphs) {
    if (!L->isLexingRawMode())
        L->Diag(CP-2, 0/*diag::trigraph_ignored*/);
    return 0;
  }

  if (!L->isLexingRawMode())
      L->Diag(CP-2, 0/*diag::trigraph_converted*/) << StringRef(&Res, 1);
  return Res;
}
...
Slash:
    ...
    if (unsigned EscapedNewLineSize = getEscapedNewLineSize(Ptr)) {
      ...
      if (Ptr[0] != '\n' && Ptr[0] != '\r' && Tok && !isLexingRawMode())
          Diag(Ptr, 0/*diag::backslash_newline_space*/);
