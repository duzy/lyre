...
#include "lyre/frontend/TextDiagnostic.h"
#include "lyre/base/CharInfo.h"
#include "lyre/base/DiagnosticOptions.h"
#include "lyre/base/FileManager.h"
...
TextDiagnostic::emitDiagnosticMessage(SourceLocation Loc,
    ...
    StringRef Message,
    ArrayRef<lyre::CharSourceRange> Ranges,
    ...
    bool CLFallbackMode) 
{
    ...
    // In lyre-cl /fallback mode, print diagnostics as "error(lyre):". This
    // makes it more clear whether a message is coming from lyre or cl.exe,
    ...
    const SourceManager &SM) 
{
    ...
    case DiagnosticOptions::Lyre: OS << ':'  << LineNo; break;
    ...
    if (DiagOpts->ShowSourceRanges && !Ranges.empty()) {
        ...
        for (ArrayRef<CharSourceRange>::const_iterator RI = Ranges.begin(),
            ...
            unsigned TokSize = 0;
            if (RI->isTokenRange())
                TokSize = 0; //Lexer::MeasureTokenLength(E, SM, LangOpts);
    ...
    const LangOptions &LangOpts) 
{
    ...
    if (EndLineNo == LineNo) {
        ...
        if (EndColNo) {
            ...
            if (R.isTokenRange())
                EndColNo += 0; //Lexer::MeasureTokenLength(End, SM, LangOpts);
        } else {
    ...
    const SourceManager &SM) 
{
    ...
    for (ArrayRef<FixItHint>::iterator I = Hints.begin(), E = Hints.end();
        ...
        if (I->RemoveRange.isTokenRange())
            EInfo.second += 0; //Lexer::MeasureTokenLength(ELoc, SM, LangOpts);
